...
void
exportLastPositionsAndVelocities(const std::vector<Superboid>& superboids, const step_int step)
{
  ...
  for (auto& super : superboids)
  {
    ...
    for (auto& mini : super.miniboids)
    {
      for (auto& component : mini.position)
        binaryOutFile.write(reinterpret_cast<const char *>(&component), sizeof(real));
...
SCS::write(const step_int step, const std::vector<Superboid>& superboids)
{
  for (auto& super : superboids)
  {
    ...
    SCS::file() << std::fixed << step << '\t' << \
      super.ID << '\t' << \
      getModule(super.miniboids[0u].position, peripheralsCM) << std::endl;
  }
  return;
}
