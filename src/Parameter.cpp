...
std::vector<Parameter<std::string>>
    Parameter<std::string>::map = std::vector<Parameter<std::string>>();
template <>
...
std::vector<Parameter<real>>
    Parameter<real>::map = std::vector<Parameter<real>>();
template <>
...
template <typename P> bool Parameter<P>::has(const std::string &n) {
...
template <typename P> Parameter<P> Parameter<P>::get(const std::string &n) {
  for (const auto &elem : map)
    if (elem.name == n)
      return elem;
...
template <typename P> const P &Parameter<P>::operator()(void) const {
...
template <typename P> void Parameter<P>::operator=(const P &p) {
...
bool SuperParameter::areDependenciesSet(void) {
  ...
  for (std::size_t depIndex = 0; depIndex < this->_dependencies.size();
    ...
    if (Parameter<unsigned long int>::has(this->_dependencies[depIndex]))
      if (!Parameter<unsigned long int>::get(this->_dependencies[depIndex])
               .isSet())
        ++counter[depIndex];
    ...
    if (Parameter<std::vector<real>>::has(this->_dependencies[depIndex]))
      if (!Parameter<std::vector<real>>::get(this->_dependencies[depIndex])
               .isSet())
        ++counter[depIndex];

    if (Parameter<std::vector<std::vector<real>>>::has(
            this->_dependencies[depIndex]))
      if (!Parameter<std::vector<std::vector<real>>>::get(
               this->_dependencies[depIndex])
               .isSet())
        ++counter[depIndex];
  }
      ...
      if (parameter.name == keys[0]) {
        ...
        if (keys.size() == 1) {
          if (values.size() == 1 && isNumeric(values[0]))
            parameter = std::vector<std::vector<real>>(
                elementsNo,
                std::vector<real>(elementsNo, std::stof(values[0])));
...
std::string getParametersSample() {
  ...
  for (const auto &param : Parameter<std::string>::map)
    stream << param.name << getSpaces(charsNo - param.name.size()) << '=' << ' '
           << param.defaultValue << std::endl;
